...
ENTRYPOINT ["sh", "-c", "java -Dfile.encoding=UTF-8 -Duser.timezone=UTC -javaagent:/app/opentelemetry-javaagent.jar -Dotel.service.name=leoni-backendv2 -Dotel.exporter.otlp.endpoint=https://ingest.us2.signoz.cloud:443 -Dotel.exporter.otlp.headers=signoz-ingestion-key=${SIGNOZ_ACCESS_TOKEN} -Dotel.exporter.otlp.protocol=http/protobuf -Dotel.traces.exporter=otlp -Dotel.metrics.exporter=otlp -Dotel.logs.exporter=otlp -jar app.jar"]
